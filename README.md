...
A/H twins are the same company listed on both Shanghai/Shenzhen (the "A" share, CNY) and Hong Kong (the "H" share, HKD). Frictions between the two markets — capital controls, mainland retail participation, quota limits on Stock Connect — keep the two prices from tracking mechanically, so the A/H premium mean-reverts around a slowly-drifting equilibrium. This project selects tradeable pairs from the scraped Hang Seng A/H list, fits a time-varying hedge ratio with a Kalman filter, and trades a rolling z-score of the residual spread with realistic per-leg costs.
...
- **Costs** — round-trip bps per leg, applied as half on each weight change: base case 100 bps on the A-share leg (mainland stamp duty + commission + slippage) and 60 bps on the H-share leg. Full grid in the Results section.
...
Breakeven A-leg round-trip cost (H held at 0.6× A): **~144 bps full sample, ~223 bps in-sample, ~77 bps out-of-sample**. The OOS breakeven sits below the base cost assumption — turnover ~14× gross a year means every extra 10 bps of round-trip cost shifts net Sharpe by roughly 0.2–0.3 units.
